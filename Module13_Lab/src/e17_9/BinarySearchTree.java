...
public class BinarySearchTree
	...
	*/
    ...
    private static boolean depthFirst(Node node, Visitor v) {
        if (node == null) {
            return true; // Continue traversing
        }

        boolean shouldContinue = v.visit(node.data);
        if (!shouldContinue) {
            return false; // Stop traversing
        }

        // Continue with left and right child only if shouldContinue is true
        return depthFirst(node.left, v) && depthFirst(node.right, v);
    }
	class Node
	{
